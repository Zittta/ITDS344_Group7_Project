...
def silver_transform_crime(**context) -> dict:
    ...
    for doc in cursor:
        ...
        offense_cat = (doc.get("offense_category") or "").strip().upper() or "UNKNOWN"
...
def gold_dim_time(**context) -> dict:
    """
    Extracts unique (date, hour) keys from silver_crime_clean → gold.dim_time.
    Uses $setOnInsert so concurrent writes from seattle_911_pipeline are safe.
    """
    client    = MongoClient(MONGO_URI, serverSelectionTimeoutMS=30_000)
    db_silver = client[SILVER_DB]
    ...
    for doc in db_silver["silver_crime_clean"].find(
        {"report_date_time": {"$ne": None}}, {"report_date_time": 1}
    ):
        dt = doc.get("report_date_time")
    ...
    for tk in time_keys:
        ...
        if len(ops) >= CHUNK_SIZE:
            _bulk_upsert(coll, ops, "dim_time-crime")
            ops = []

    _bulk_upsert(coll, ops, "dim_time-crime")
    log.info("[Gold] dim_time (crime): %d time keys", len(time_keys))
    client.close()
    return {"time_keys": len(time_keys)}


# ─── Task 3b: Gold — dim_location ────────────────────────────────────────────
...
# ─── Task 5a: Gold — agg_crime_by_neighborhood_month ─────────────────────────

def gold_agg_crime_by_neighborhood(**context) -> dict:
    """Materialises crime count per neighborhood per month (full rebuild)."""
    client  = MongoClient(MONGO_URI, serverSelectionTimeoutMS=30_000)
    db_gold = client[GOLD_DB]
    ...
    count = db_gold["agg_crime_by_neighborhood_month"].count_documents({})
    log.info("[Gold] agg_crime_by_neighborhood_month: %d rows", count)
    client.close()
    return {"agg_crime_by_neighborhood_month": count}


# ─── Task 5b: Gold — agg_crime_by_offense_category ───────────────────────────
...
with DAG(
    ...
    t_dim_time = PythonOperator(
        ...
        doc_md="Upsert (date, hour) keys from silver_crime_clean into gold.dim_time",
    )
    ...
    t_fact = PythonOperator(
        ...
        doc_md="Build gold.fact_crime_events — incremental, joins dim_time + dim_location + dim_offense",
    )
    t_agg_hood = PythonOperator(
        task_id="gold_agg_crime_by_neighborhood",
        python_callable=gold_agg_crime_by_neighborhood,
        doc_md="Materialise agg_crime_by_neighborhood_month (full rebuild)",
    )
    ...
    # Bronze → Silver → [dim_time, dim_location, dim_offense] → fact → [agg x3]
    t_bronze >> t_silver >> [t_dim_time, t_dim_loc, t_dim_off] >> t_fact >> [t_agg_hood, t_agg_cat, t_agg_pc]
